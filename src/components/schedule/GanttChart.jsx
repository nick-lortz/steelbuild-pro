...
export default function GanttChart({ 
  ...
  const togglePhase = (phaseKey) => {
    ...
    if (newCollapsed.has(phaseKey)) newCollapsed.delete(phaseKey);
    else newCollapsed.add(phaseKey);
  ...
  while (current <= endDate) {
    ...
    if (viewMode === 'day') current = addDays(current, 1);
    else if (viewMode === 'week') current = addDays(current, 7);
  ...
  const tasksByProjectAndPhase = useMemo(() => {
    ...
    activeTasks.forEach(task => {
      ...
      if (groupByWBS) {
        ...
        if (!grouped[projectId][wbs]) grouped[projectId][wbs] = [];
        grouped[projectId][wbs].push(task);
      } else {
        const phase = task.phase || 'unassigned';
        if (!grouped[projectId][phase]) grouped[projectId][phase] = [];
  ...
  const getTaskPosition = (task) => {
    ...
    try {
      ...
      if (isNaN(taskStart.getTime()) || isNaN(taskEnd.getTime())) return { left: '0%', width: '0%' };
  ...
  const hasRFIImpact = (task) => task.linked_rfi_ids && task.linked_rfi_ids.length > 0;
  const hasCOImpact = (task) => task.linked_co_ids && task.linked_co_ids.length > 0;
  ...
  const isOverdue = (task) => {
    ...
    try {
      ...
      return isPast(end) && isBefore(end, new Date());
    } catch {
      return false;
    }
  };
  ...
  const handleDragEnd = async (e) => {
    ...
    try {
      ...
      const percentage = relativeX / chartRef.current.scrollWidth;
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (todayPosition >= 0 && todayPosition <= 100) scrollToToday();
    }, 500);
                ...
                <div key={projectId} className="border-b border-zinc-800">
                  ...
                  {!isProjectCollapsed && Object.entries(phaseGroups)
                    ...
                    .map(([groupKey, groupTasks]) => {
                      ...
                      // ✅ FIX: no React.Fragment here (prevents invalid props warning)
                      return (
                        <div key={groupId}>
                          {/* Phase/WBS Header */}
                                ...
                                <div className="flex border-b border-zinc-800 hover:bg-zinc-800/40 group transition-colors">
                                  ...
                                  <div className="flex-1 relative py-2 overflow-hidden" style={{ minWidth: `${periods.length * columnWidth}px` }}>
                                    ...
                                    <div
                                      ...
                                      onClick={(e) => {
                                        if (!draggingTask) handleTaskClick(task, e); // ✅ pass event
                                      }}
                                      onContextMenu={(e) => {
                                        e.preventDefault();
                                        setEditingDependencies(task);
                                    ...
                                    {task.predecessor_ids?.filter(predId => filteredTasks.some(t => t.id === predId)).map(predId => {
                                      ...
                                      // ✅ avoid React.Fragment here too (same warning risk)
                                      return (
                                        <div key={predId} className="contents">
                                          {/* Horizontal line */}
                                          <div
                                    ...
                                    <div key={childTask.id} className="flex border-b border-zinc-800/50 hover:bg-zinc-800/30 group transition-colors">
                                      ...
                                      <div className="flex-1 relative py-2 overflow-hidden" style={{ minWidth: `${periods.length * columnWidth}px` }}>
                                        ...
                                        <div
                                          ...
                                          onClick={(e) => {
                                            if (!draggingTask) handleTaskClick(childTask, e); // ✅ pass event
                                          }}
            ...
            {completedTasks.length > 0 && (
              <>
                ...
                {!collapsedCompleted && Object.entries(completedTasksByProject).map(([projectId, projectCompletedTasks]) => {
                  const proj = projects.find(p => p.id === projectId) || { name: 'Unassigned', project_number: 'N/A' };
      ...
      <AlertDialog open={!!deleteTask} onOpenChange={() => setDeleteTask(null)}>
        <AlertDialogContent className="bg-zinc-900 border-zinc-800">
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={() => {
                if (deleteTask && onTaskDelete) onTaskDelete(deleteTask.id);
                setDeleteTask(null);
              }}
