...
export default function DetailingRevisionHistory({ detailingId, projectId }) {
  const queryClient = useQueryClient();

  const { data: revisions = [], isLoading } = useQuery({
    queryKey: ['detailing-revisions', detailingId],
    queryFn: async () => {
      const results = await base44.entities.DetailingRevision.filter(
  ...
  const revertMutation = useMutation({
    mutationFn: async ({ revisionId, snapshotData, revisionNumber }) => {
      const snapshot = JSON.parse(snapshotData);
      
      // Update the parent Detailing entity with snapshot data
      await base44.entities.Detailing.update(detailingId, {
        ...snapshot,
      ...
      const newRevision = await base44.entities.DetailingRevision.create({
        detailing_id: detailingId,
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries(['detailing-revisions', detailingId]);
      queryClient.invalidateQueries(['detailing', detailingId]);
    },
    onError: (error) => {
